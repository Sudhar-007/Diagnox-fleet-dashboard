...
export default function ServiceHistory({ records, showTruck = false }) {
  ...
  if (!records) return <p className="text-[15px] text-muted">Loading service records…</p>;
  if (records.length === 0) {
    return <p className="text-[15px] text-muted">No service records yet. Use Add service record to log work done.</p>;
  }
