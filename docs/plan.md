...
- [ ] 5. SOS lifecycle + global SOS banner + Scenario panel + Pipeline strip
...
- [ ] 12. `fastapi` / `hybrid` source wiring against the real public FastAPI endpoint (validate/coerce numeric fields in source.js; strip incoming keys that collide with derived fields; evaluate every ingested point in time order, not only the newest per batch, so a backlog uploaded after a 4G outage still raises its alerts)
